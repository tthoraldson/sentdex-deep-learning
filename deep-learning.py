...
# ^ an example of a feed forward neural network
#
...
x = tf.placeholder('float', [None, 784]) # if you don't add [None, 784], an error
y = tf.placeholder('float')              # will not be thrown if not the same shape
...
def train_neural_network(x):
    ...
    optimizer = tf.Variable(tf.train.AdamOptimizer().minimize(cost))
